...
const customerAndAge = (obj) => {
    ...
    for (const name in obj) {
        let str = `Customer Name :${name} , Age :${obj[name]}`;
        newArr.push(str); 
    }
    return newArr;
};
...
const getInfo = (arr) => {
    let coursesName = [];
    let studentsName = [];
    for (const courseObj of arr) {
        coursesName.push(courseObj.course);
        for (const studentStr in courseObj.Students) {
            studentsName.push(studentStr);
        }
    }
...
const getStudents = (arr) => {
    let studentsInfo = [];
    for (const lostStudent of arr) {
        let newObj = {};
        for (const courseObj of courses) {
            if (courseObj.Students.includes(lostStudent)) {
                newObj["Student"] = lostStudent;
