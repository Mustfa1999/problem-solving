...
function square(arr) {
    return arr.map(function(i) {return i*i;} );
}
...
function fullName(arr) {
    return arr.map(function(i){return `${i.firstName} ${i.lastName}`;});
}
...
function gradesAvg(arr) {
    ...
    arr.map(function(i) { i["avg"] = average(i.gradsList); } );
    return arr;
}
...
function studentsResult(arr) {
    let newArr = gradesAvg(arr);
    newArr.map(function(i) { (i.avg >= 50.0) ? i["result"] = "Passed" : i["result"] = "Failed"; } );
    return newArr;
}
